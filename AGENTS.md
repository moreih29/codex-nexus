...
Source of truth: installed `@moreih29/nexus-core` published CLI contract, `docs/plugin-guide.md`, `docs/contract/harness-io.md`, and the Codex scaffold outputs generated by `bun run sync:core`.
...
2. Review local `node_modules/@moreih29/nexus-core/package.json`, `docs/plugin-guide.md`, `docs/contract/harness-io.md`, and `docs/plugin-template/codex/README.md` for contract changes.
3. Run `bun run sync:core` to refresh wrapper-consumed managed outputs from the published Codex sync contract.
4. Inspect the generated diffs before editing wrapper code.
5. Update Codex-specific wrapper layers (`src/cli/*`, `src/config/*`, hook/MCP adapters) only where the new core contract requires it.
6. Run `bun run check` and `bun test` before concluding the upgrade.
