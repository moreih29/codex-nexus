...
- 테스트가 inline `config.toml [hooks]` 표면과 legacy `.codex/hooks.json` fallback 표면을 둘 다 검증해야 한다
...
- hook 호환성 스모크는 **실제로 어떤 표면이 검증됐는지** 함께 기록한다
  - 테스트 머신의 `codex-cli` 가 `0.124.0` 미만이거나 버전을 확인할 수 없으면, live smoke는 legacy `.codex/hooks.json` fallback 표면만 직접 검증한다
  - inline `config.toml [hooks]` 표면은 자동 테스트로 계속 커버하고, 가능하면 `codex-cli >= 0.124.0` 환경에서 추가 smoke를 한 번 더 수행한다
...
1. `npx -y codex-nexus@<target-version> install --scope user`
   - 단, publish 전에는 위 명령 대신 temp package root override 방식으로 같은 동작을 검증한다
2. `npx -y codex-nexus@<target-version> doctor --scope user`
3. 아래 파일 직접 확인
   - `~/.codex/config.toml`
   - `~/.codex/hooks.json` (legacy 표면이 선택됐는지 확인하거나, inline install에서 managed hook이 남지 않았는지 확인할 때)
...
- codex-nexus managed hook이 `.codex/config.toml` inline `[hooks]` 와 `.codex/hooks.json` 양쪽에 동시에 들어가지 않았는가
- hook 표면 선택이 아래 규칙과 일치하는가
  - 기존 inline `[hooks]`가 있던 환경은 inline을 유지
  - inline `[hooks]`가 없고 기존 `.codex/hooks.json`이 있던 환경은 legacy를 유지
  - 새 환경에서 `codex-cli >= 0.124.0` 이면 inline `[hooks]`
  - 버전 미확인 또는 `codex-cli < 0.124.0` 이면 `.codex/hooks.json` fallback
- active 표면이 inline이면 managed `SessionStart` / `UserPromptSubmit` / `PreToolUse` / `PermissionRequest` / `Stop` hook이 `config.toml`에만 있고, managed `hooks.json` copy는 생기지 않았는가
- active 표면이 legacy면 managed hook이 `.codex/hooks.json`에 있고, `config.toml` 안에 managed inline hook이 같이 생기지 않았는가
...
3. 아래 파일 직접 확인
   ...
   - `<repo>/.codex/hooks.json` (legacy 표면이 선택됐는지 확인하거나, inline install에서 managed hook이 남지 않았는지 확인할 때)
...
- codex-nexus managed hook이 `<repo>/.codex/config.toml` inline `[hooks]` 와 `<repo>/.codex/hooks.json` 양쪽에 동시에 들어가지 않았는가
- hook 표면 선택이 아래 규칙과 일치하는가
  - 기존 inline `[hooks]`가 있던 환경은 inline을 유지
  - inline `[hooks]`가 없고 기존 `.codex/hooks.json`이 있던 환경은 legacy를 유지
  - 새 환경에서 `codex-cli >= 0.124.0` 이면 inline `[hooks]`
...
- codex-nexus managed hook이 inline `[hooks]` 와 `.codex/hooks.json` 양쪽에 동시에 기록됨
